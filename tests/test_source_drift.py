...
class SourceDriftTests(unittest.TestCase):
    ...
    def test_piccoma_canary_rejects_mutated_product_signals(self):
        product_html = (FIXTURES_ROOT / "piccoma" / "normal" / "01-product.html").read_text(
            encoding="utf-8"
        )
        mutations = (
            (
    ...
    <script type="application/ld+json">
      {"@type":"Product","name":"九条の大罪"}
                ...
                "LD JSON Product.name not found",
            ),
            (
                "non_product_ld_json",
                0,
        ...
        """,
                "#js_episodeList latest episode identifier not found",
            ),
        )

        for mutation, step_index, body, expected_message in mutations:
            with self.subTest(mutation=mutation):
                manifest, client = load_fixture_http_client("piccoma", "normal")
                client.steps[step_index] = {
                    "url": manifest["steps"][step_index]["url"],
                    "body": body,
                }
