...
- merger: comic_crawler 用の最終 merge gate。`$spacex-chief-reviewer` の `APPROVE` コメント、全 review thread resolved、`Residual Risks` の issue/sub-issue 化を確認し、満たすときだけ merge する。Use when: comic_crawler の PR を条件付きで自動マージしたいとき。 (file: ./.agents/skills/merger/SKILL.md)
