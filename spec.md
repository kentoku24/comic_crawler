...
`latest` query は、初期応答と本文応答の 2 段階で扱ってよい。

- 初期応答として、認識した元メッセージにチェックマークのスタンプ送信を試行してよい
- スタンプ送信の試行は本文応答生成より前に行ってよい
- スタンプ送信は受領確認のための best-effort とし、送信保証や再試行を要求しない
- スタンプ送信失敗は `latest` 本文応答の失敗理由にしてはならない
- スタンプ送信結果待ちによって `latest` 本文応答を不必要に遅延させてはならない
...
- 初期応答として、認識した元メッセージにチェックマークのスタンプ送信を試行できること
- スタンプ送信の試行は background run 起動より前に行ってよい
- `fetch` の初期応答は本文返信ではなく上記スタンプで表現してよい
- スタンプ送信は受領確認のための best-effort とし、送信保証や再試行を要求しない
- スタンプ送信失敗は `fetch` run 起動の失敗理由にしてはならない
- スタンプ送信結果待ちによって `fetch` run 起動を不必要に遅延させてはならない
