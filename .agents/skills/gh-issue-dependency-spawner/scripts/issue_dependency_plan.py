...
def main() -> None:
    ...
    child_section = extract_section(parent["body"], CHILD_SECTION_NAMES)
    ...
    order_hint = extract_order_hint(extract_section(parent["body"], ORDER_SECTION_NAMES))
    children = {
        number: fetch_issue(owner, repo, number)
        for number in child_numbers
    }

    raw_edges = extract_mermaid_edges(
        extract_section(parent["body"], DEPENDENCY_SECTION_NAMES)
    )
