...
class ComicDaysAdapter(SourceAdapter):
    source = "comic-days"

    def can_handle(self, seed_url: str) -> bool:
        return bool(
            parse_comic_days_episode_url(seed_url)
            or parse_comic_days_series_feed_url(seed_url)
        )

    def normalize(self, seed_url: str) -> WorkDescriptor:
        normalized_episode_url = parse_comic_days_episode_url(seed_url)
        ...
        feed_match = parse_comic_days_series_feed_url(seed_url)
        if not feed_match:
            raise RuntimeError(f"comic-days: unsupported seed URL: {seed_url}")
        ...
        return WorkDescriptor(
            ...
            seed_url=canonical_comic_days_series_feed_url(series_id),
            metadata={
    ...
    def canonicalize_item(
        ...
        series_id = str(item.get("seriesId") or "") or extract_comic_days_series_id_from_seed_url(seed_url)
        if series_id:
            return self.normalize(canonical_comic_days_series_feed_url(series_id))

        episode_url = parse_comic_days_episode_url(seed_url)
        if not episode_url:
            raise RuntimeError("comic-days: unsupported seed URL")

        episode_html = http_client.get_text(episode_url)
        feed_url = extract_comic_days_series_feed_url(episode_html)
        if feed_url:
            return self.normalize(feed_url)

        series_id = extract_comic_days_series_id(episode_html)
        if not series_id:
            raise RuntimeError("comic-days: series id not found")
        return self.normalize(canonical_comic_days_series_feed_url(series_id))
    ...
    def fetch_latest(self, work: WorkDescriptor, http_client: HttpClient) -> LatestEpisode:
        ...
        feed_match = parse_comic_days_series_feed_url(work.seed_url)
        if feed_match:
            feed_url = canonical_comic_days_series_feed_url(feed_match[1])
            series_id = series_id or feed_match[1]
        else:
            episode_url = parse_comic_days_episode_url(work.seed_url)
            if not episode_url:
                raise RuntimeError("comic-days: unsupported seed URL")
            episode_html = http_client.get_text(episode_url)
            feed_url = extract_comic_days_series_feed_url(episode_html)
            series_id = series_id or extract_comic_days_series_id(episode_html) or ""
            if not series_id:
                raise SourceParseError("comic-days: series id not found")
            if not feed_url:
                raise SourceParseError("comic-days: series feed URL not found")
            series = f"{self.source}:{series_id}"

        feed_text = http_client.get_text(feed_url)
        latest_url, episode_title, series_title = parse_comic_days_feed_latest(feed_text)
        page_title = html_title(http_client.get_text(latest_url))
        parsed_episode_title, parsed_series_title = parse_comic_days_title(page_title or "")
